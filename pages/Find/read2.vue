		...
		methods: {
			...
			**/
			getDirectoryList(showLoading) {
				console.log("===>加载getDirectoryList")
				/*****************************************/
				/**********    根据需要更改    ************/
				/*****************************************/
						...
						uniCloud.callFunction({
							...
							for(let i=1;i<=res.result.data.length;i++){
								this.textList.push({
									...
									text: res.result.data[i-1].article_content
								})
								this.directoryList.push({
									index: i-1,
									chapterId: i,   //注意：这个chapterId用于获取章节内容而不是index
									name: `第${i}章`+res.result.data[i-1].article_name
								})
							}
							this.getThreeChapter(this.history.chapterIndex)
						})
						resolve()
					}, 300)
				}).then(() => {}, ()=> {
